...
class TransactionGraphVisualizer:
    ...
    def _create_legend_traces(self):
        ...
        legend_items = [
            ('🚫 Banned', self.NODE_COLORS['banned']),
            ('🔴 Fraudster', self.NODE_COLORS['fraudster']),
            ('💰 Fraud (Dirty)', self.NODE_COLORS['fraud_dirty']),
            ('🧼 Fraud (Clean)', self.NODE_COLORS['fraud_clean']),
            ('🤖 Bot', self.NODE_COLORS['bot']),
            ('💼 Entrepreneur', self.NODE_COLORS['entrepreneur']),
            ('👔 Worker', self.NODE_COLORS['worker']),
            ('🎓 Student', self.NODE_COLORS['student']),
            ('👤 Civilian', self.NODE_COLORS['civilian']),
            ('❓ Unknown', self.NODE_COLORS['unknown']),
        ]
    ...
    def visualize_html(self, output_path='transaction_graph.html'):
        ...
        edge_trace = go.Scatter(x=edge_x, y=edge_y, mode='lines',
                                line=dict(width=1, color='#888'), hoverinfo='none', opacity=0.5,showlegend=False)
        ...
        fig = go.Figure(
            ...
            layout=go.Layout(
                ...
                legend=dict(
                    ...
                    bordercolor='#ddd',
                    borderwidth=2,
                    title=dict(text='📊 Node Types', font=dict(size=14, family='Arial'))
                ),
